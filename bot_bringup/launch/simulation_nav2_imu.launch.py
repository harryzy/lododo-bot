#!/usr/bin/env python3
"""
IMU Navigation Simulation Launch File / IMU 导航仿真启动文件
Complete simulation with Wheel Odometry + IMU fusion for navigation
完整的轮式里程计 + IMU 融合导航仿真

Architecture / 架构:
    ┌─────────────────────────────────────────────────────────────┐
    │                   simulation_nav2_imu.launch.py             │
    │                                                             │
    │  ┌─────────────┐    ┌─────────────┐    ┌─────────────┐     │
...
Startup Sequence (Event-Driven) / 启动顺序（事件驱动）:
    ...
    Phase 4: Navigation
        - Start SLAM Toolbox (using camera for mapping only)
...
def launch_setup(context: LaunchContext, *args, **kwargs):
    ...
    slam_config = os.path.join(pkg_bot_navigation, 'config', 'slam_toolbox_imu.yaml')
    ...
    event_robot_spawned = RegisterEventHandler(
        OnProcessExit(
            ...
            on_exit=[
                ...
                LogInfo(msg='[Phase 2] Starting robot control, perception, and IMU relay...'),
                omni_controller,
    ...
    # Event: When perception launch completes -> start Phase 3 (IMU Fusion)
    # 事件：当感知启动完成后 -> 启动第三阶段（IMU融合）
